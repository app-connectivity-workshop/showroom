...
All the services in the namespace, including the `travels` REST API, can access `discounts`. However, only the backend services `cars`, `flights`, `hotels`, and `insurances` should be permitted to do so.

In this section, you'll apply an *Istio AuthorizationPolicy*, that enables access control on workloads in the mesh, to restrict access to the `discounts` service, enforcing a least privilege model

== Verify direct access of *discounts* service

. Run the following command in the *upper teminal* to verify that you are indeed able to access the `discounts` service directly from `travels` pod. This command calls the `discounts` services directly from the `travels` pod. 
+
...
Your goal is to restrict access to the `discounts` service so that only specific backend services (such as `cars` and `flights) can access it it, while preventing access from the `travels` REST API. This is achieved using an Istio Custom Resource (CR) `AuthorizationPolicy`.
