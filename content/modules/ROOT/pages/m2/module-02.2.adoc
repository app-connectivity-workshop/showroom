...
The micoservices within the `travel-agency` are able to communicate freely. While this simplifies development early on, it doesn't align with security best practices, especially when some services handle sensitive logic or customer data. One of the services - the `discounts` service -  is responsible for calculating and applying pricing discounts based on proprietary business rules. Access to this service must be tightly controlled.
...
Currently, any service in the `travel-agency` namespace—including frontend aggregators like `travels`—can communicate with the `discounts` service.

However, only specific backend services—`cars`, `flights`, `hotels`, and `insurances`—should be allowed to call `discounts`.
