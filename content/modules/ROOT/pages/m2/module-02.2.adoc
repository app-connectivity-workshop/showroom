...
The microservices in the `travel-agency` namespace can currently communicate freely. While this makes development cycle easier, it poses security risks especially for services like `discounts` which applies proprietary pricing logic and handles sensitive data.

image:m2/travel-agency-namespace-initial.png[width=70%] 

As it stands, all the services in the namespace, including the `travels-agency` REST API, can access `discounts`. However, only the backend services `cars`, `flights`, `hotels`, and `insurances` should be permitted to do so.
...
. Run the following command in the *upper teminal* to verify that you are indeed able to access the `discounts` service directly from `travels` pod.
+
